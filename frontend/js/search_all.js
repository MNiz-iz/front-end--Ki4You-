...
const name_search = localStorage.getItem('name_search');
...
xhr.onreadystatechange = () => {
    if (xhr.readyState === XMLHttpRequest.DONE){
        ...
        //i = 0;
        console.log(res)
        //console.log(res.tree[0].Trees.name)

        //console.log(res.tree.length);
        let main = document.querySelector('#main');
        ...
        if(res == 'data not found') {
            htmls += `
            <div class="alert">
                <span class="closebtn" onclick="redirect()">&times;</span> 
                <strong>แจ้งเตือน!</strong> ขณะนี้ไม่มีต้นไม้ชื่อเดียวกับที่ค้นหาในฐานข้อมูล.
            </div>
                `
        } else {
        for (var i = 0; i < res.tree.length; i++){
            var object = res.tree[i].Trees
            //
...
const url_endpoint = api_url + `/tree_data/search/${name_search}`
...
function call_page(index){
    ...
    window.open(url_front + '/template/search_tree.html');
}
